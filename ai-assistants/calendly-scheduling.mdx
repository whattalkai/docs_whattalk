...
Your Calendly event type's location setting must be compatible with the voice assistant.
...
<Warning>
If your event type uses only video conferencing (Google Meet, Zoom, Teams), bookings will fail. The voice assistant cannot generate meeting links. Either switch to "Custom" or add it as an additional location option.
</Warning>
